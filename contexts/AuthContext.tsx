...
import { AuthProviderPropsI } from "@/components/interface";

export const AuthContext = createContext({});

export const useAuth = () => useContext(AuthContext);

export default function AuthProvider({ children }: AuthProviderPropsI) {
  const { initTransactionState, setTxId, setTransactionStatus }: any =
    useTransaction();
  ...
  const loadProfile = useCallback(async () => {
    const profile = await fcl.query({
      ...
      `,
      args: (arg, t: any) => [arg(currentUser.addr, t.Address)],
    });
  ...
  const createProfile = async () => {
    ...
    const transactionId = await fcl.mutate(txnDetails1);
  ...
  const updateProfile = async ({ name, color, info }: any) => {
    ...
    const transactionId = await fcl.mutate(txnDetails2);
    setTxId(transactionId);
    fcl.tx(transactionId).subscribe((res) => {
      setTransactionStatus(res.status);
      if (res.status === 4) {
